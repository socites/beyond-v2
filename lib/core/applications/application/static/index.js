...
module.exports = function (application, config, runtime) {
    ...
    if (config.indexOf('styles.css') === -1) {
        config.push('styles.css');
    }
    if (config.indexOf('bootstrap.css') === -1) {
        config.push('bootstrap.css');
    }
    ...
    this.resource = async(function* (resolve, reject, key, language) {
        ...
        if (!(yield finder.exists(key))) {
            ...
            if (['index.html', 'styles.css', 'bootstrap.css'].indexOf(key) !== -1) {

                let process = require('./process.js');
                let file = require('path').join(__dirname, 'artifacts', key);
