...
module.exports = function (root, specs) {
    ...
    Object.defineProperty(this, 'items', {
        'get': function () {
            if (!keys) throw new Error('collection must be processed before accessing this property');
            return items;
        }
    });
    ...
    this.exists = async(function* (resolve, reject, key) {
        ...
        let sep = require('path').sep;
        if (sep !== sep) key = key.replace(new RegExp('\\' + sep, 'g'), '/');
    ...
    this.process = async(function* (resolve, reject) {
        ...
        for (let i in files) {
            ...
            let sep = require('path').sep;
            if (sep !== '/') key = key.replace(new RegExp('\\' + sep, 'g'), '/');
