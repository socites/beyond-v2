require('colors');
module.exports = require('async')(function *(resolve, reject, application, dirname, file) {
    "use strict";

    file = require('path').resolve(dirname, file);

    let fs = require('fs');
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {

        let message = 'Invalid overwrites configuration on application "'.red +
